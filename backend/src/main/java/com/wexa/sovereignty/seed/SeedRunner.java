...
/**
 * Loads the demo scenario. Run with:
 *
 *   ./mvnw spring-boot:run -Dspring-boot.run.profiles=seed
 *
 * Everything is MERGE-based, so re-running refreshes properties in place
 * instead of duplicating data. Constraints and indexes are created before any
 * data is written — on a 256 MB c0 instance lookups should hit those, not scans.
 */
@Component
@Profile("seed")
public class SeedRunner implements ApplicationRunner {
    ...
    public SeedRunner(Driver driver, ConfigurableApplicationContext context) {
        this.driver = driver;
    ...
    private void logCounts() {
        try (Session session = driver.session()) {
            List<Record> counts = session.executeRead(tx -> tx.run(COUNTS).list());
            counts.forEach(row -> log.info("  {}: {}", row.get("label").asString(), row.get("count").asLong()));
        }
    }
}
